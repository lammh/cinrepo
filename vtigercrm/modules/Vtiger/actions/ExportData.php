...
global $log;
$log->debug("Entering ./actions/ExportData.php::process");
...
global $log;
$log->debug("Entering ./actions/ExportData.php::ExportData");
...
global $log;
$log->debug("Entering ./actions/ExportData.php::getExportQuery");
...
global $log;
$log->debug("Entering ./actions/ExportData.php::getExportContentType");
...
global $log;
$log->debug("Entering ./actions/ExportData.php::output");
		$moduleName = $request->get('source_module');
...
global $log;
$log->debug("Entering ./actions/ExportData.php::sanitizeValues");
		$db = PearDatabase::getInstance();
		$currentUser = Users_Record_Model::getCurrentUserModel();
		...
		foreach($arr as $fieldName=>&$value){
			...
			if($fieldname != 'hdnTaxType' && ($uitype == 15 || $uitype == 16 || $uitype == 33)){
				...
				if($uitype == 33 || $uitype == 16 || in_array($value,$this->picklistValues[$fieldname])){
					...
					$value = trim($value);
				} else if (array_key_exists ($value , $this->picklistValues[$fieldname])){
					$value = trim($this->picklistValues[$fieldname][$value]);
				} else {
					$value = '';
				}
