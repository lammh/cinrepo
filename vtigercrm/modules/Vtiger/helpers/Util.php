...
class Vtiger_Util_Helper {
	...
	public static function getRecordName ($recordId, $checkDelete=false) {
        global $log;
        $log->debug("Entering ./helpers/Util.php::staticgetRecordName");
        ...
        if($num_rows) {
            $log->debug("Exiting ./helpers/Util.php::staticgetRecordName");
    ...
    public static function getPickListValues($fieldName) {
        global $log;
        $log->debug("Entering ./helpers/Util.php::staticgetPickListValues");
        ...
        $log->debug("Exiting ./helpers/Util.php::staticgetPickListValues");
        return $values;
    }
	...
	public static function getBaseCurrency() {
        global $log;
        $log->debug("Entering ./helpers/Util.php::staticgetBaseCurrency");
        $db = PearDatabase::getInstance();
		$result = $db->pquery('SELECT * FROM vtiger_currency_info WHERE defaultid < 0', array());
		if($db->num_rows($result)) {
            $log->debug("Exiting ./helpers/Util.php::staticgetBaseCurrency");
    ...
    public static function getRoleBasedPicklistValues($fieldName, $roleId) {
        global $log;
        $log->debug("Entering ./helpers/Util.php::staticgetRoleBasedPicklistValues");
        $db = PearDatabase::getInstance();
        ...
        $log->debug("Exiting ./helpers/Util.php::staticgetRoleBasedPicklistValues");
    ...
    public static function getLabel($recordId , $ignoreDelete=true){
        global $log;
        $log->debug("Entering ./helpers/Util.php::staticgetLabel");
        $db = PearDatabase::getInstance();
        $query = 'SELECT label from vtiger_crmentity WHERE crmid=?';
        if($ignoreDelete) {
            $query .= ' AND deleted=0';
        ...
        $log->debug("Exiting ./helpers/Util.php::staticgetLabel");
        return $name;
    }
	...
	public static function getCurrentInfoOfUser() {
        global $log;
        $log->debug("Entering ./helpers/Util.php::staticgetCurrentInfoOfUser");
        $db = PearDatabase::getInstance();
		$currentUser = Users_Record_Model::getCurrentUserModel();
	    $result = $db->pquery('SELECT * FROM vtiger_currency_info WHERE id = ?', array($currentUser->get('currency_id')));
		if($db->num_rows($result))  {
            $log->debug("Exiting ./helpers/Util.php::staticgetCurrentInfoOfUser");
            return $db->query_result_rowdata($result, 0);
        }
        $log->debug("Exiting ./helpers/Util.php::staticgetCurrentInfoOfUser");
	}
	...
	public static function setCalendarDefaultActivityTypesForUser($userId) {
        global $log;
        $log->debug("Entering ./helpers/Util.php::staticsetCalendarDefaultActivityTypesForUser");
		$db = PearDatabase::getInstance();
		...
		if($db->num_rows($userEntries) <= 0) {
			...
			foreach($activityIds as $activityId=>$color) {
				$db->pquery(
                    ...
                    array($db->getUniqueID('vtiger_calendar_user_activitytypes'), $activityId, $userId, $color)
                );
			}
		}
        $log->debug("Exiting ./helpers/Util.php::staticsetCalendarDefaultActivityTypesForUser");
	}
    ...
    public static function isUserDeleted($userid) {
        global $log;
        $log->debug("Entering ./helpers/Util.php::staticisUserDeleted");
        $db = PearDatabase::getInstance();
        $result = $db->pquery('SELECT deleted FROM vtiger_users WHERE id = ? AND (status=? OR deleted=?)', array($userid, 'Inactive', 1));
        $count = $db->num_rows($result);
        if($count > 0) {
            $log->debug("Exiting ./helpers/Util.php::staticisUserDeleted");
            return true;
        }
        $log->debug("Exiting ./helpers/Util.php::staticisUserDeleted");
        return false;
