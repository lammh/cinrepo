...
class URL extends \Workflow\Attachment {
    ...
    public function getConfigurations($moduleName) {
        $configuration = array(
            'html' => '<a href="#" onclick="attachURLFile(); ">Attach File from URL</a>
                <div id="urlAttachmentContainer" style="display:none;border:1px solid #ccc;margin:5px;padding:10px;">
                    Load file from this URL and attach it to the E-Mail.<br/>
                    <div class="insertTextfield" style="display:inline;" data-name="attachment_url" data-style="width:200px;" data-id="attachment_url"></div>
                    <br><br>Filename of this file:<br/><em>(leave empty to get filename from downloaded file)</em><br/>
                    <div class="insertTextfield" style="display:inline;" data-name="attachment_name" data-style="width:200px;" data-id="attachment_name"></div>
                    <div class="buttonbar" style="text-align:center;margin-top:5px;">
                        <input type="button" class="btn btn-warning" onclick="jQuery(\'#urlAttachmentContainer\').hide();" value="Cancel">
                        <input type="button" class="btn btn-primary" onclick="addURLAttachmentCallback();" value="Insert">
                    </div>
                </div>

            ',
            'script' => "
function attachURLFile() {
    jQuery('#urlAttachmentContainer').show();
}
function addURLAttachmentCallback() {
    jQuery('#urlAttachmentContainer').hide();

    var url = jQuery('#attachment_url').val();
    var name = jQuery('#attachment_name').val();

    Attachments.addAttachment('s#url#' + url + '#' + name, name == ''?'filename from url':name, '',{name:name,val:url});
}
        ");
