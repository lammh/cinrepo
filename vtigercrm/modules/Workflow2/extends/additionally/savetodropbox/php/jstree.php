...
function generateChildrenTree($folders) {
    $result = array();

    foreach($folders as $fileObject) {
        $children = array();
        if($fileObject->is_dir) {
            $tmp = array(
                "attr" => array("id" => "node_".md5( $fileObject->path), "path" => trim($fileObject->path)),
                "data" => basename($fileObject->path),
                "state" => "closed"
            );

            #$tmp["children"] = generateChildrenTree($folder["children"]);
...
switch($_REQUEST["operation"]) {
    case "get_children":
        $folders = $dropbox->metaData($_GET["path"]);

        $jsonResult = generateChildrenTree($folders['body']->contents);

    break;
    case "rename_node":
        #DocumentWrapper::renameFolder($_REQUEST["id"], $_REQUEST["title"]);

        $jsonResult = array("status" => 1);
        break;
    case "move_node":
        #DocumentWrapper::moveFolder($_REQUEST["id"], $_REQUEST["ref"]);
...
//
//        $resultID = DocumentWrapper::createFolder($parent_id, $title);

        $jsonResult = array("status" => 1, "id" => $resultID, "path" => $_REQUEST["parentpath"]."/".$title);
        break;
/*    case "create":
