...
$field->name = 'hr_net_price';  #fieldname 日本語ダメ
$field->table = $module->basetable; #vtiger_salesorder
$field->column = 'hr_net_price'; #入力フォームの項目名(columnname, fieldLabel)日本語ダメ
...
/** START 割引 ****************/
#$field = new Vtiger_Field();
#$field->name = 'hr_discount';  #fieldname 日本語ダメ
#$field->table = $module->basetable; #vtiger_salesorder
#$field->column = 'hr_discount'; #入力フォームの項目名(columnname, fieldLabel)日本語ダメ
#$field->columntype = 'INT(11)';
#$field->uitype = 7;  #7_数値
#$field->typeofdata = 'I~O';
#$field->displaytype= 1;
...
/** START マージン率合計 ***/
#$field = new Vtiger_Field();
#$field->name = 'margin_rate_total';  #fieldname 日本語ダメ
#$field->table = $module->basetable; #vtiger_salesorder
#$field->column = 'margin_rate_total'; #入力フォームの項目名(columnname, fieldLabel)日本語ダメ
#$field->columntype = 'DECIMAL(25,3)';
#$field->uitype = 9;  #9_パーセンテージ
...
/** START リクルート請求 *****/
#$field = new Vtiger_Field();
#$field->name = 'hr_claim_flag';  #fieldname 日本語ダメ
#$field->table = $module->basetable; #vtiger_salesorder
#$field->column = 'hr_claim_flag'; #入力フォームの項目名(columnname, fieldLabel)日本語ダメ
...
#'リクルート(R請求)','リクルート(R請求)','リクルート(新卒・CIN請求）','インテリジェンス(an/DODA）','マイナビ','ディップ(バイトル)','ネオキャリア(@type/女の転職)','イーキャリア','クリエイト','求人ジャーナル','アスコム','案内広告社(DOMO)','毎日エージェンシー(アイデム)','マージン調整分','その他(備考欄へ)'
#) );
#/** END   版元 ******/
#
#/** START 請求希望日 ******/
