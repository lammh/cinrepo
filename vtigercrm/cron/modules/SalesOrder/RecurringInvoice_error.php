...
$sql .= " payment_duration, invoice_status FROM ";
$sql .= " vtiger_salesorder ";	#受注データ
$sql .= " INNER JOIN vtiger_crmentity "; #モジュール情報
$sql .= " ON vtiger_salesorder.salesorderid = vtiger_crmentity.crmid AND vtiger_crmentity.deleted = 0 ";
$sql .= " INNER JOIN vtiger_invoice_recurring_info ";	#請求書の発行方法
$sql .= " ON vtiger_salesorder.salesorderid = vtiger_invoice_recurring_info.salesorderid ";
$sql .= " WHERE DATE_FORMAT(start_period,'%Y-%m-%d') <= '".date('Y-m-d')."' AND DATE_FORMAT(end_period,'%Y-%m-%d') >= '".date('Y-m-d')."'";
...
for($i=0; $i<$no_of_salesorder;$i++) {
	...
	$salesorder_id = $adb->query_result($result, $i,'salesorderid');	#レコードID
	$start_period = $adb->query_result($result, $i,'start_period');		#開始時期
	$end_period = $adb->query_result($result, $i,'end_period');			#終了時期
	$last_recurring_date = $adb->query_result($result, $i,'last_recurring_date');	#
	$recurring_frequency = $adb->query_result($result, $i,'recurring_frequency');	#自動で計上、請求書を作成する周期, weekly, monthly

	if ($last_recurring_date == NULL  || $last_recurring_date == '' || $last_recurring_date == '0000-00-00') {	#last_recurring_dateのデータはNULL,空,0000-00-00のとき
		$last_recurring_date = $start_period;	#開始時期を$last_recurring_dateに格納
	}//End if

    $currentDate = date('Y-m-d');	#当日の年月日を格納
    if(strtotime($last_recurring_date) < strtotime($currentDate)){	#$last_recurring_dateが$currentDate(当日の年月日)より小さいとき
        $last_recurring_date = $currentDate;	#$currentDate(当日の年月日)を格納
    }//End if

	$currentRecurringDate = $last_recurring_date;
	if(strtotime($currentRecurringDate) > strtotime($end_period)) {	#$currentRecurringDateより終了時期が小さいとき
		continue;
	}//End if
...
print "<p>currentRecurringDate:".$currentRecurringDate." == currentDate:".$currentDate."</p>";

#    if(strtotime($currentRecurringDate) == strtotime($currentDate)) {
#print "<p> | createInvoice</p>";
...
function createInvoice($salesorder_id) {
	...
	print "ooooo : ";
	var_dump($focus->id);
	var_dump($focus->column_fields['invoiceid']);
