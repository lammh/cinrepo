...
Vtiger_Edit_Js("Inventory_Edit_Js",{
	...
	setTotalAfterDiscount : function(lineItemRow, totalAfterDiscountValue){
		console.log('ddd'+totalAfterDiscountValue);
	...
	//大熊追加 151214
	setSOSalesMount : function(netTotalValue) {
		//売上額の反映
		$('#SalesOrder_editView_fieldName_salesamount').val(netTotalValue);
	},

	//大熊追加 151214
	setIVSalesMount : function(netTotalValue) {
		//売上額の反映
		$('#Invoice_editView_fieldName_salesamount').val(netTotalValue);
	},
	...
	calculateTotalAfterDiscount: function(lineItemRow) {
		...
		console.log('ccc'+totalAfterDiscount);
	...
	calculateNetTotal : function() {
		...
		lineItemTable.find('tr.'+this.rowClass).each(function(index,domElement){
			...
			console.log('fff'+thisInstance.getLineItemNetPrice(lineItemRow));
			netTotalValue += thisInstance.getLineItemNetPrice(lineItemRow);
		});
		console.log('eee'+netTotalValue);
		this.setNetTotal(netTotalValue);

		//SalesOrderへの売上額の反映		
		this.setSOSalesMount(netTotalValue);	//大熊 151204
		//Invoiceへの売上額の反映
		this.setIVSalesMount(netTotalValue);	//大熊 151204

		//原価合計の取得
		var cost_total  = this.getSOCostTotal(netTotalValue);	//大熊 151204
		if(isNaN(cost_total)) {
			var cost_total 	    = this.getIVCostTotal(netTotalValue);	//大熊 151204
		}//End if
		if(isNaN(cost_total)) cost_total = 0;
		console.log('cost_total'+cost_total);

		//粗利の取得
		var amount = netTotalValue - cost_total;	//大熊 151204
		//粗利の反映
		this.setSOAmount(amount);	//大熊 151204
		this.setIVAmount(amount);	//大熊 151204
	},

	//原価合計の取得	大熊追加 151214
	getSOCostTotal : function(netTotalValue) {
		//紹介率
		var introduction_fee_rate = parseFloat($('#SalesOrder_editView_fieldName_costinfo').val());

		//紹介料
		if(introduction_fee_rate) {
			var introduction_fee = parseInt(netTotalValue*(introduction_fee_rate/100));
		...
		return(introduction_fee+costall+costsyanai);
	},

	//原価合計の取得	大熊追加 151214
	getIVCostTotal : function(netTotalValue) {

		//紹介料
		var introduction_fee = parseInt($('#Invoice_editView_fieldName_introduction_fee').val());
		//社外原価
		var costall          = parseInt($('#Invoice_editView_fieldName_cost_customer').val());
		//社内原価
//		var costsyanai       = parseInt($('#Invoice_editView_fieldName_costsyanai').val());

		return(introduction_fee+costall);
	},

	//粗利の反映	大熊追加 151214
	setSOAmount : function(amount) {
		//売上額の反映
		$('#SalesOrder_editView_fieldName_amount').val(amount);	//大熊追加 151214
	},

	//粗利の反映	大熊追加 151214
	setIVAmount : function(amount) {
		//売上額の反映
		$('#Invoice_editView_fieldName_amount').val(amount);	//大熊追加 151214
	},
