...
<table class="table table-bordered table-report">
    <thead>
        <tr class="blockHeader">
           <th colspan="2">
                {vtranslate('LBL_SCHEDULE_EMAIL',$MODULE)} <input type="checkbox" name="isReportScheduled" id="isReportScheduled" {if $IS_SCHEDULED eq 'true'} checked {/if}>
           </th>
       </tr>
    </thead>
    <tbody> 
                ...
                <div class="row-fluid">    
                    <div class="span" id="scheduledTypeSpan" >
                        <select class="chzn-select  chzn-done" name="scheduledType" id="scheduledType" onchange="javascript: setScheduleOptions();">
                                ...
                                {*<option id="schtype_1" value="1" {if $schtypeid eq 1}selected{/if}>{'Hourly'|@getTranslatedString:'ITS4YouReports'}</option>*}
	...
	<tr>
            ...
            <td>
                ...
                <select id="scheduledReportFormat" name="scheduledReportFormat" class="small">
                        <option value="pdf" {if $REPORT_FORMAT eq 'pdf'} selected {/if}>{vtranslate('LBL_REPORT_FORMAT_PDF',$MODULE)}</option>
                        <option value="excel" {if $REPORT_FORMAT eq 'excel'} selected {/if}>{vtranslate('LBL_REPORT_FORMAT_EXCEL',$MODULE)}</option>
                        <option value="csv" {if $REPORT_FORMAT eq 'csv'} selected {/if}>{vtranslate('LBL_REPORT_FORMAT_CSV',$MODULE)}</option>
                        <option value="xml" {if $REPORT_FORMAT eq 'xml'} selected {/if}>{vtranslate('LBL_REPORT_FORMAT_XML',$MODULE)}</option>
                        <option value="every" {if $REPORT_FORMAT eq 'every'} selected {/if}>{vtranslate('LBL_REPORT_FORMAT_EVERY',$MODULE)}</option>
                </select>*}
            </td>
	</tr>
	<tr>
            <td class="fieldLabel medium"><label class="pull-right marginRight10px">{vtranslate('LBL_USERS_AVAILABEL',$MODULE)}</label></td>
            <td>
                {vtranslate('LBL_SELECT',$MODULE)}:&nbsp;
