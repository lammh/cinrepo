...
{strip}
  ...
  {* フィールドの取得方法⇒　 $RECORD_STRUCTUR["ブロックのラベル名"]["フィールド名"]->最終的な要素　　*}
  
  {*$RECORD_STRUCTURE["LBL_DATAINFO"]["conf1_dates"]|@debug_print_var*}
  {assign var=IS_HIDDEN2 value=$RECORD_STRUCTURE["LBL_DATAINFO"]["sostatus"]->fieldvalue}
			...
			{else}
				...
				{else}
				 ...
				 <td class="fieldValue {$WIDTHTYPE}" id="{$MODULE_NAME}_detailView_fieldValue_{$FIELD_MODEL->getName()}" {if $FIELD_MODEL->get('uitype') eq '19' or $FIELD_MODEL->get('uitype') eq '20'} colspan="3" {assign var=COUNTER value=$COUNTER+1} {/if}>
					...
					{if $IS_AJAX_ENABLED && $FIELD_MODEL->isEditable() eq 'true' && ($FIELD_MODEL->getFieldDataType()!=Vtiger_Field_Model::REFERENCE_TYPE) && $FIELD_MODEL->isAjaxEditable() eq 'true' && substr($IS_HIDDEN2,0,1) neq '2'}
