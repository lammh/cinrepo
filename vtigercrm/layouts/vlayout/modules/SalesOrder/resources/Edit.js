...
jQuery( function() {
	...
	jQuery('#SalesOrder_editView_fieldName_costall') . change( function () {
	  	var cost_all = parseInt($('#SalesOrder_editView_fieldName_costall').val());
	  	if(isNaN(cost_all)) cost_all = 0;
	    var salesamount = parseInt($('#SalesOrder_editView_fieldName_salesamount').val());
	  	if(isNaN(salesamount)) salesamount = 0;
	  	var amount = salesamount - cost_all;
	...
	//発注先原価 1	を入力したとき
	jQuery('#SalesOrder_editView_fieldName_order_cost_01') . change( function () {
		calcGross();
	} );

	//発注先原価 2	を入力したとき
	jQuery('#SalesOrder_editView_fieldName_order_cost_02') . change( function () {
		calcGross();
	} );

	//発注先原価 3	を入力したとき
	jQuery('#SalesOrder_editView_fieldName_order_cost_03') . change( function () {
		calcGross();
	} );

	//発注先原価 4	を入力したとき
	jQuery('#SalesOrder_editView_fieldName_order_cost_04') . change( function () {
		calcGross();
	} );

	//発注先原価 5	を入力したとき
	jQuery('#SalesOrder_editView_fieldName_order_cost_05') . change( function () {
		calcGross();
	} );

	//WSの原価の合計
	function get_ws_cost_total() {
		var total_cost = 0;
		var order_cost;
		for(i=1;i<=5;i++) {
			order_cost = 0;
			order_cost = parseInt($('#SalesOrder_editView_fieldName_order_cost_0'+i).val());
			if(isNaN(order_cost)) continue;
...
//		return sumGrossRate;
  ...
  function calcGross(gross_rate) {
    ...
    //WSの原価の取得
    var ws_cost = get_ws_cost_total();

		console.log("ws_cost : "+ws_cost);
		...
		//社外原価
		var cost_fee = (ws_cost);

		console.log("cost_fee : "+cost_fee);
		//社内原価
		var costsyanai = parseInt($('#SalesOrder_editView_fieldName_costsyanai').val().split(",").join(""));
	  ...
	  if(isNaN(introduction_fee)) introduction_fee = 0;
		...
		var cost_fee = (introduction_fee + costsyanai + cost_fee);

    if(isNaN(cost_fee)) cost_fee = 0;

		console.log("introduction_fee : "+introduction_fee);
		
		console.log("costsyanai"+costsyanai);
		console.log("cost_fee"+cost_fee);
		if(cost_fee !== 0) {
			amount -= cost_fee;
		}//End if
...
//		$('#SalesOrder_editView_fieldName_costall').val(cost_fee);
		$('#SalesOrder_editView_fieldName_amount').val(amount);
