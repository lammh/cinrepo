...
<div id="breadcrumb">             
    <ul class="crumbs marginLeftZero">
        <li class="first step {if $STEP eq "1"}active{/if}" style="z-index:10;" id="steplabel1"><a><span class="stepNum">1</span><span class="stepText">{if $TYPE eq "reactivate" }{vtranslate('LBL_REACTIVATE','ITS4YouReports')}{else}{vtranslate('LBL_VALIDATION','ITS4YouReports')}{/if}</span></a></li>
        {if $TOTAL_STEPS eq "4"}
        <li class="step {if $STEP eq "2"}active{/if}" style="z-index:9;"  id="steplabel2"><a><span class="stepNum">2</span><span class="stepText">{vtranslate('LBL_IMPORT_STEP','ITS4YouReports')}</span></a></li>    
        {/if}    
        {if $TOTAL_STEPS eq "4"}
        <li class="step {if $STEP eq "3"}active{/if}" style="z-index:9;"  id="steplabel2"><a><span class="stepNum">3</span><span class="stepText">{vtranslate('LBL_DOWNLOAD_STEP','ITS4YouReports')}</span></a></li>    
        {/if}    
        <li class="step last {if $CURRENT_STEP eq $TOTAL_STEPS}active{/if}" style="z-index:7;"  id="steplabel{$TOTAL_STEPS}"><a><span class="stepNum">{$TOTAL_STEPS}</span><span class="stepText">{vtranslate('LBL_FINISH','ITS4YouReports')}</span></a></li>
    </ul>
</div>
<div class="clearfix">
</div>
...
<div id="step1" class="padding1per" style="border:1px solid #ccc; {if $STEP neq "1"}display:none;{/if}" >     
        ...
        <div>
            <strong>
                {if $TYPE eq "reactivate" }
                    {vtranslate('LBL_MISMATCH','ITS4YouReports')}</strong>
                {else}
                    {vtranslate('LBL_WELCOME','ITS4YouReports')}</strong>
                {/if}
        </div>
        <br>
        <div class="clearfix">
    ...
    <div class="controls">
        <div>
            {if $TYPE eq "reactivate" }
                {vtranslate('LBL_MISMATCH_DESC','ITS4YouReports')}
            {else}
           {vtranslate('LBL_WELCOME_DESC','ITS4YouReports')}</br>

            {vtranslate('LBL_WELCOME_FINISH','ITS4YouReports')}
            {/if}
        </div>
        </br>
<div class="span8 alert alert-danger container-fluid" style="float:left;margin-left: 0px;">{vtranslate('LBL_LICENSE_CONDITIONS','ITS4YouReports')}</div>

        <br>
        <div class="clearfix">
        </div>
    </div>   
{if $TYPE neq "reactivate" }
    <div class="controls">
        <div>
            <strong>{vtranslate('LBL_CHECK_COMPANY','ITS4YouReports')}</strong>
        </div>
        <div class="clearfix">
        </div>
    </div>
    <div class="controls">
        <div>
            <strong>{vtranslate('LBL_INSERT_KEY','ITS4YouReports')}</strong>
        </div>
        <div>
            {vtranslate('LBL_ONLINE_ASSURE','ITS4YouReports')}
        </div>
        <div class="clearfix">
        </div>
    </div> 
{/if}
    ...
    <div class="controls">
        ...
        <div style="text-align:center;">
            <!--{vtranslate('LBL_LICENSE_KEY','ITS4YouReports')}:&nbsp;<input type="text" class="input-large" id="licensekey" name="licensekey" data-validation-engine="validate[required]"/>-->
            {if $TYPE eq "reactivate" }
                <button type="submit" id="reactivate_license" class="btn btn-success"/><strong>{vtranslate('LBL_REACTIVATE','ITS4YouReports')}</strong></button>&nbsp;&nbsp;
            {else}
                <button type="submit" id="validate_button" class="btn btn-success"/><strong>{vtranslate('LBL_VALIDATE','ITS4YouReports')}</strong></button>&nbsp;&nbsp;
            {/if}
            {if $TYPE neq "reactivate" }
                <button type="button" id="order_button" class="btn btn-info" onclick="window.location.href='http://www.its4you.sk/en/vtiger-shop.html'"/>{vtranslate('LBL_ORDER_NOW','ITS4YouReports')}</button>
            {/if}
        </div>
        <div class="clearfix">
        </div>
