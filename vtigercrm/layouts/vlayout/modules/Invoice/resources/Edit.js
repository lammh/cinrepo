...
jQuery( function() {
    ...
    function introductionReflection() {
        ...
        //粗利の計算
        amount = salesamount-(introduction_fee+ws_cost);
        //粗利の反映
...
*/
    ...
    //発注先原価 1   を入力したとき
    jQuery('#Invoice_editView_fieldName_order_cost_01') . change( function () {
        calcGross();
    } );

    //発注先原価 2   を入力したとき
    jQuery('#Invoice_editView_fieldName_order_cost_02') . change( function () {
        calcGross();
    } );

    //発注先原価 3   を入力したとき
    jQuery('#Invoice_editView_fieldName_order_cost_03') . change( function () {
        calcGross();
    } );

    //発注先原価 4   を入力したとき
    jQuery('#Invoice_editView_fieldName_order_cost_04') . change( function () {
        calcGross();
    } );

    //発注先原価 5   を入力したとき
    jQuery('#Invoice_editView_fieldName_order_cost_05') . change( function () {
        calcGross();
    } );
    ...
    function get_ws_cost_total() {
        var total_cost = 0;
        for(i=1;i<=5;i++) {
            total_cost += parseInt($('#Invoice_editView_fieldName_order_cost_0'+i).val());
        }//End for
        return total_cost;
    }//End function
    ...
    function calcGross(grossRate) {
        ...
        var ws_cost = get_ws_cost_total();
        $('#Invoice_editView_fieldName_cost_customer').val(ws_cost);
        ...
        if(salesamount == "") {
	        ...
	        salesamount = parseInt(salesamount.split(",").join(""));
            if(grossRate !== 0) {
                amount = Math.round(salesamount*grossRate);
            } else {
                amount = salesamount;
            }//End if
            amount -= ws_cost;
