...
<script type="text/javascript">
    ...
    function change_modulesorce(first, second_name) {ldelim}
        if (selected_module != '') {ldelim}
    ...
    function fillModuleFields(first, second_name) {ldelim}
        ...
        for (b = 0; b < blocks_length; b += 2) {ldelim}
            optGroup = document.createElement('optgroup');
            optGroup.label = blocks[b];
            box2.appendChild(optGroup);
            var list = module_fields[module + '|' + blocks[b + 1]];
            for (i = 0; i < list.length; i += 2) {ldelim}
                objOption = document.createElement("option");
