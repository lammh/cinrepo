...
#					$field->getColumnName()." = $tableName.id";
				$sql .= " $tableJoinMapping[$tableName] $tableName ON vtiger_crmentity.smownerid = $tableName.id ";	# 大熊追加 151016
			} elseif($tableName == 'vtiger_groups') {
				$field = $moduleFields[$ownerField];
#				$sql .= " $tableJoinMapping[$tableName] $tableName ON ".$field->getTableName().".".
#					$field->getColumnName()." = $tableName.groupid";
				$sql .= " $tableJoinMapping[$tableName] $tableName ON vtiger_crmentity.smownerid = $tableName.groupid ";	# 大熊追加 151016
			} else {
				$sql .= " $tableJoinMapping[$tableName] $tableName ON $baseTable.".
					"$baseTableIndex = $tableName.$moduleTableIndexList[$tableName]";
			}
		...
		/** ▼ Potentials ****/
		if( $this->meta->getTabName() == 'Potentials') {
			/** ▼ ** creater 151014 大熊追加 *****/
			$tableJoinCondition['creater'] = array(
				'vtiger_users_creater'=>"$baseTable.creater = vtiger_users_creater.id"
			);
			$tableJoinMapping['vtiger_users_creater'] = 'LEFT JOIN vtiger_users';
			/** ▲ ** creater 151014 大熊追加 *****/

			/** ▼ ** transactor 大熊追加 *****/
			$tableJoinCondition['transactor'] = array(
				'vtiger_users_transactor'=>"$baseTable.transactor = vtiger_users_transactor.id"
			);
			$tableJoinMapping['vtiger_users_transactor'] = 'LEFT JOIN vtiger_users';
			/** ▲ ** transactor 大熊追加 *****/
		}//End if
		/** ▲ Potentials ****/

		/** ▼ SalesOrder ****/
		if( $this->meta->getTabName() == 'SalesOrder') {
			/** ▼ ** technique_representative 大熊追加 *****/
			$tableJoinCondition['technique_representative'] = array(
				'vtiger_users_technique_representative'=>"$baseTable.technique_representative = vtiger_users_technique_representative.id"
			);
			$tableJoinMapping['vtiger_users_technique_representative'] = 'LEFT JOIN vtiger_users';
			/** ▲ ** technique_representative 大熊追加 *****/
		}//End if
		/** ▲ SalesOrder ****/

		/** ▼ Invoice ****/
		if( $this->meta->getTabName() == 'Invoice') {
			/** ▼ ** technique_representative 大熊追加 *****/
			$tableJoinCondition['technique_representative'] = array(
				'vtiger_users_technique_representative'=>"$baseTable.technique_representative = vtiger_users_technique_representative.id"
			);
        ...
        foreach ($this->conditionals as $index=>$conditionInfo) {
			...
			foreach ($valueSqlList as $valueSql) {
				if (in_array($fieldName, $this->referenceFieldList)) {
                    ...
                    } elseif($fieldName == 'creater') {
                    	/** ▼ creater 151014大熊追加 ***/
						$concatSql = getSqlForNameInDisplayFormat(array('first_name'=>"vtiger_users_creater.first_name",'last_name'=>"vtiger_users_creater.last_name"), 'Users');
						$fieldSql .= "$fieldGlue (trim($concatSql) $valueSql or "."vtiger_groups.groupname $valueSql)";
                    	/** ▲ creater151014大熊追加  ***/
                    } elseif($fieldName == 'transactor') {
                    	/** ▼ transactor 151014大熊追加 ***/
						$concatSql = getSqlForNameInDisplayFormat(array('first_name'=>"vtiger_users_transactor.first_name",'last_name'=>"vtiger_users_transactor.last_name"), 'Users');
						$fieldSql .= "$fieldGlue (trim($concatSql) $valueSql or "."vtiger_groups.groupname $valueSql)";
                    	/** ▲ transactor 151014大熊追加 ***/
                    } elseif($fieldName == 'technique_representative') {
                    	/** ▼ technique_representative 151014大熊追加 ***/
						$concatSql = getSqlForNameInDisplayFormat(array('first_name'=>"vtiger_users_technique_representative.first_name",'last_name'=>"vtiger_users_technique_representative.last_name"), 'Users');
						$fieldSql .= "$fieldGlue (trim($concatSql) $valueSql or "."vtiger_groups.groupname $valueSql)";
                    	/** ▲ technique_representative 151014大熊追加 ***/
                    }else{
