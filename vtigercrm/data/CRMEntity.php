...
class CRMEntity {
	...
	static function getInstance($module) {
        global $log;
        $log->debug("Entering CRMEntity::getInstance(".$module.") method ...");
		$modName = $module;
        ...
        $log->debug("Exiting getInstance(".$module.") method ...");
		return $focus;
	}
	...
	function uploadAndSaveFile($id, $module, $file_details) {
        ...
        $log->debug("Entering CRMEntity::uploadAndSaveFile(".$id.",".$module.",".$file_details.") method ...");
		...
		if ($save_file == 'true' && $upload_status == 'true') {
            ...
            $log->debug("Exiting uploadAndSaveFile(".$id.",".$module.",".$file_details.") method ...");
			return true;
		} else {
	...
	function insertIntoCrmEntity($module, $fileid = '') {
        ...
        $log->debug("Entering CRMEntity::insertIntoCrmEntity(".$module.",".$fileid.") method ...");
		if ($fileid != '') {
			$this->id = $fileid;
			$this->mode = 'edit';
		}
        ...
        $log->debug("Exiting insertIntoCrmEntity(".$module.",".$fileid.") method ...");
	}

	// Function which returns the value based on result type (array / ADODB ResultSet)
	protected function resolve_query_result_value($result, $index, $columnname) {
		global $adb;
        global $log;
        $log->debug("Entering CRMEntity::resolve_query_result_value(".$result.",".$index.", ".$columnname.") method ...");        
		if (is_array($result)) {
            $log->debug("Exting CRMEntity::resolve_query_result_value(".$result.",".$index.", ".$columnname.") method ...");        
	 ...
	 * @param $module -- module:: Type varchar
	 */ // here is the table to be modified
	function insertIntoEntityTable($table_name, $module, $fileid = '') {
		global $log;
		global $current_user, $app_strings;
		global $adb;

        $log->debug("Entering CRMEntity::insertIntoEntityTable(".$table_name.", ".$module.", ".$fileid.") method ...");        
		$insertion_mode = $this->mode;
        ...
        $log->debug("Exting CRMEntity::insertIntoEntityTable(".$table_name.", ".$module.", ".$fileid.") method ...");        
	}

    static function putlog($arg) {
        global $log;
        $log->debug("Entering ./data/CRMEntity.php::staticputlog");
	 ...
	 * @param $table_name -- table name:: Type varchar
	 * The function will delete a record .The id is obtained from the class variable 
     * $this->id and the columnname got from $this->tab_name_index[$table_name]
	 */
	 ...
	 * The function will get the attachmentsid for the given entityid from vtiger_seattachmentsrel table 
     * and get the attachmentsname from vtiger_attachments table returns the 'filename'
	 */
	function getOldFileName($notesid) {
		global $log;

        $log->debug("Entering CRMEntity::getOldFileName(".$notesid.") method ...");        
        ...
        $log->debug("Exting CRMEntity::getOldFileName(".$notesid.") method ...");        
		return $filename;
	}
	...
	function retrieve_entity_info($record, $module) {
        ...
        $log->debug("Entering CRMEntity::retrieve_entity_info(".$record.",".$module.") method ...");        
        ...
        $log->debug("Exting CRMEntity::retrieve_entity_info(".$record.",".$module.") method ...");        
	}
	...
	function save($module_name, $fileid = '') {
        ...
        $log->debug("Entering CRMEntity::save(".$module_name.", ".$fileid.") method ...");        
        ...
        $log->debug("Exting CRMEntity::save(".$module_name.", ".$fileid.") method ...");        
	}
	...
	function process_list_query($query, $row_offset, $limit = -1, $max_per_page = -1) {
        ...
        $log->debug("Entering CRMEntity::process_list_query(".$query.", ".$row_offset.", ".$limit.", ".$max_per_page.") method ...");        
        ...
        $log->debug("Exting CRMEntity::process_list_query(".$query.", ".$row_offset.", ".$limit.", ".$max_per_page.") method ...");        
		return $response;
	}
	...
	function process_full_list_query($query) {
        ...
        $log->debug("Entering CRMEntity::process_full_list_query(".$query.") method ...");        
		...
		if (isset($list)) {
            $log->debug("Exting CRMEntity::process_full_list_query(".$query.") method ...");        
			return $list;
		} else {
            $log->debug("Exting CRMEntity::process_full_list_query(".$query.") method ...");        
			return null;
	...
	function mark_deleted($id) {
        ...
        $log->debug("Entering CRMEntity::mark_deleted(".$id.") method ...");        
		$date_var = date("Y-m-d H:i:s");
        ...
        $log->debug("Exting CRMEntity::mark_deleted(".$id.") method ...");        
	}

	function retrieve_by_string_fields($fields_array, $encode = true) {
        global $log;

        $log->debug("Entering CRMEntity::retrieve_by_string_fields(".$fields_array.", ".$encode.") method ...");        
		$where_clause = $this->get_where($fields_array);
		$query = "SELECT * FROM $this->table_name $where_clause";
		$this->log->debug("Retrieve $this->object_name: " . $query);
		$result = & $this->db->requireSingleResult($query, true, "Retrieving record $where_clause:");
		if (empty($result)) {
            $log->debug("Exting CRMEntity::retrieve_by_string_fields(".$fields_array.", ".$encode.") method ...");        
			return null;
		}
	...
	function checkIfCustomTableExists($tablename) {
        ...
        $log->debug("Entering CRMEntity::checkIfCustomTableExists(".$tablename.") method ...");        
        ...
        $log->debug("Exting CRMEntity::checkIfCustomTableExists(".$tablename.") method ...");        
		return $exists;
	}
	...
	function constructCustomQueryAddendum($tablename, $module) {
        ...
        $log->debug("Entering CRMEntity::constructCustomQueryAddendum(".$tablename.", ".$module.") method ...");        
		$tabid = getTabid($module);
        ...
        $log->debug("Exting CRMEntity::constructCustomQueryAddendum(".$tablename.", ".$module.") method ...");        
		return $sql3;
	}
	...
	function get_full_list($order_by = "", $where = "") {
        global $log;
        $log->debug("Entering CRMEntity::get_full_list(".$order_by.",".$where.") method ...");        
		$log->debug("get_full_list:  order_by = '$order_by' and where = '$where'");
		$query = $this->create_list_query($order_by, $where);
        $log->debug("Exting CRMEntity::get_full_list(".$order_by.",".$where.") method ...");        
	...
	function apply_field_security($moduleName = '') {
        ...
        $log->debug("Entering CRMEntity::apply_field_security(".$moduleName.") method ...");        
		if($moduleName == '') {
        ...
        $log->debug("Exting CRMEntity::apply_field_security(".$moduleName.") method ...");        
	}
	...
	function initImportableFields($module) {
        ...
        $log->debug("Entering CRMEntity::initImportableFields(".$module.") method ...");        
		require_once('include/utils/UserInfoUtil.php');
        ...
        $log->debug("Exting CRMEntity::initImportableFields(".$module.") method ...");        
	}
	...
	function initRequiredFields($module) {
        ...
        $log->debug("Entering CRMEntity::initRequiredFields(".$module.") method ...");        
		$tabid = getTabId($module);
        ...
        $log->debug("Exting CRMEntity::initRequiredFields(".$module.") method ...");        
	}
	...
	function trash($module, $id) {
        ...
        $log->debug("Entering CRMEntity::trash(".$module.", ".$id.") method ...");        
		if(!self::isBulkSaveMode()) {
            require_once("include/events/include.inc");
            $em = new VTEventsManager($adb);
        ...
        $log->debug("Exting CRMEntity::trash(".$module.", ".$id.") method ...");        
	}
	...
	function unlinkDependencies($module, $id) {
        ...
        $log->debug("Entering CRMEntity::unlinkDependencies(".$module.",".$id.") method ...");        
        ...
        $log->debug("Exting CRMEntity::unlinkDependencies(".$module.",".$id.") method ...");        
	}
	...
	function unlinkRelationship($id, $return_module, $return_id) {
        ...
        $log->debug("Entering CRMEntity::unlinkRelationship(".$id.", ".$return_module.", ".$return_id.") method ...");
        ...
        $log->debug("Exting CRMEntity::unlinkRelationship(".$id.", ".$return_module.", ".$return_id.") method ...");
	}
	...
	function restore($module, $id) {
		global $current_user, $adb, $id, $log;

        $log->debug("Entering CRMEntity::restore(".$module.", ".$id.") method ...");
        ...
        $log->debug("Exting CRMEntity::restore(".$module.", ".$id.") method ...");
	}
	...
	function restoreRelatedRecords($module, $record) {
        ...
        $log->debug("Entering CRMEntity::restoreRelatedRecords(".$module.", ".$record.") method ...");
        ...
        $log->debug("Exting CRMEntity::restoreRelatedRecords(".$module.", ".$record.") method ...");
	}
	...
	function initSortByField($module) {
        ...
        $log->debug("Entering CRMEntity::initSortByField(".$module.") method ...");
        ...
        $log->debug("Exting CRMEntity::initSortByField(".$module.") method ...");
	}
	...
	function setModuleSeqNumber($mode, $module, $req_str = '', $req_no = '') {
        ...
        $log->debug("Entering CRMEntity::setModuleSeqNumber(".$mode.", ".$module.", ".$req_str.", ".$req_no.") method ...");
		...
		if ($mode == "configure" && $req_no != '') {
			...
			if ($adb->num_rows($check) == 0) {
                ...
                $log->debug("Exting CRMEntity::setModuleSeqNumber(".$mode.", ".$module.", ".$req_str.", ".$req_no.") method ...");
				return true;
	...
	function isModuleSequenceConfigured($module) {
        ...
        $log->debug("Entering CRMEntity::isModuleSequenceConfigured(".$module.") method ...");
		$adb = PearDatabase::getInstance();
		$result = $adb->pquery('SELECT 1 FROM vtiger_modentity_num WHERE semodule = ? AND active = 1', array($module));
		if ($result && $adb->num_rows($result) > 0) {
            $log->debug("Exting CRMEntity::isModuleSequenceConfigured(".$module.") method ...");
			return true;
		}
        $log->debug("Exting CRMEntity::isModuleSequenceConfigured(".$module.") method ...");
		return false;
	...
	function getModuleSeqInfo($module) {
		global $adb, $log;
        $log->debug("Entering CRMEntity::getModuleSeqInfo(".$module.") method ...");
		$check = $adb->pquery("select cur_id,prefix from vtiger_modentity_num where semodule=? and active = 1", array($module));
		$prefix = $adb->query_result($check, 0, 'prefix');
		$curid = $adb->query_result($check, 0, 'cur_id');
        $log->debug("Exting CRMEntity::getModuleSeqInfo(".$module.") method ...");
	...
	function checkModuleSeqNumber($table, $column, $no) {
        ...
        $log->debug("Entering CRMEntity::checkModuleSeqNumber(".$table.",".$column.",".$no.") method ...");
		...
		$num_rows = $adb->num_rows($result);
		if ($num_rows > 0) {
            $log->debug("Exting CRMEntity::checkModuleSeqNumber(".$table.",".$column.",".$no.") method ...");
			return true;
		} else {
            $log->debug("Exting CRMEntity::checkModuleSeqNumber(".$table.",".$column.",".$no.") method ...");
			return false;
	...
	function updateMissingSeqNumber($module) {
        global $log, $adb;
        $log->debug("Entering CRMEntity::updateMissingSeqNumber(".$module.") method ...");

		vtlib_setup_modulevars($module, $this);
		if (!$this->isModuleSequenceConfigured($module)) {
            $log->debug("Exting CRMEntity::updateMissingSeqNumber(".$module.") method ...");
			return;
        }
	...
	function get_attachments($id, $cur_tab_id, $rel_tab_id, $actions = false) {
        ...
        $log->debug("Entering CRMEntity::get_attachments(".$id.",".$cur_tab_id.",".$rel_tab_id.",".$actions.") method ...");
        ...
        $log->debug("Exting CRMEntity::get_attachments(".$id.",".$cur_tab_id.",".$rel_tab_id.",".$actions.") method ...");
		return $return_value;
	}
	...
	function isViewed($crmid = false) {
        ...
        $log->debug("Entering CRMEntity::isViewed(".$crmid.") method ...");
		if (!$crmid) {
			$crmid = $this->id;
		}
		if ($crmid) {
			...
			if ($modifiedby == '0' && ($smownerid == $smcreatorid)) {
                ...
                $log->debug("Exting CRMEntity::isViewed(".$crmid.") method ...");
				return true;
	...
	function save_related_module($module, $crmid, $with_module, $with_crmid) {
        ...
        $log->debug("Entering CRMEntity::save_related_module(".$module.",".$crmid.",".$with_module.",".$with_crmid.") method ...");
        ...
        $log->debug("Exting CRMEntity::save_related_module(".$module.",".$crmid.",".$with_module.",".$with_crmid.") method ...");
	}
	...
	function delete_related_module($module, $crmid, $with_module, $with_crmid) {
        ...
        $log->debug("Entering CRMEntity::delete_related_module(".$module.",".$crmid.",".$with_module.",".$with_crmid.") method ...");
		if (!is_array($with_crmid))
			$with_crmid = Array($with_crmid);
		foreach ($with_crmid as $relcrmid) {
			if ($with_module == 'Documents') {
        ...
        $log->debug("Exting CRMEntity::delete_related_module(".$module.",".$crmid.",".$with_module.",".$with_crmid.") method ...");
	}
	...
	function get_related_list($id, $cur_tab_id, $rel_tab_id, $actions = false) {
        ...
        $log->debug("Entering CRMEntity::get_related_list(".$id.",".$cur_tab_id.",".$rel_tab_id.",".$actions.") method ...");
        $parenttab = getParentTab();
		$related_module = vtlib_getModuleNameById($rel_tab_id);
		$other = CRMEntity::getInstance($related_module);
		...
		if ($return_value == null)
			$return_value = Array();
		$return_value['CUSTOM_BUTTON'] = $button;
        $log->debug("Exting CRMEntity::get_related_list(".$id.",".$cur_tab_id.",".$rel_tab_id.",".$actions.") method ...");
		return $return_value;
	}
	...
	function get_dependents_list($id, $cur_tab_id, $rel_tab_id, $actions = false) {
		global $currentModule, $app_strings, $singlepane_view, $current_user, $log;

        $log->debug("Entering CRMEntity::get_dependents_list(".$id.",".$cur_tab_id.",".$rel_tab_id.",".$actions.") method ...");
		$parenttab = getParentTab();
		$related_module = vtlib_getModuleNameById($rel_tab_id);
		$other = CRMEntity::getInstance($related_module);
		...
		if ($return_value == null)
			$return_value = Array();
		$return_value['CUSTOM_BUTTON'] = $button;
        $log->debug("Exting CRMEntity::get_dependents_list(".$id.",".$cur_tab_id.",".$rel_tab_id.",".$actions.") method ...");
		return $return_value;
	}
	...
	function transferRelatedRecords($module, $transferEntityIds, $entityId) {
        ...
        $log->debug("Entering CRMEntity::transferRelatedRecords(".$module.",".$transferEntityIds.",".$entityId.") method ...");
		foreach ($transferEntityIds as $transferId) {
			// Pick the records related to the entity to be transfered, 
            // but do not pick the once which are already related to the current entity.
			...
			// Pick the records to which the entity to be transfered is related,
            // but do not pick the once to which current entity is already related.
        ...
        $log->debug("Exting CRMEntity::transferRelatedRecords(".$module.",".$transferEntityIds.",".$entityId.") method ...");
	}
	...
	function generateReportsQuery($module, $queryPlanner) {
        ...
        $log->debug("Entering CRMEntity::generateReportsQuery(".$module.",".$queryPlanner.") method ...");
		...
		$fields_query = $adb->pquery(
            "SELECT vtiger_field.fieldname,vtiger_field.tablename,vtiger_field.fieldid from vtiger_field 
             INNER JOIN vtiger_tab on vtiger_tab.name = ? 
             WHERE vtiger_tab.tabid=vtiger_field.tabid AND 
                   vtiger_field.uitype IN (10) AND
			...
			for ($i = 0; $i < $adb->num_rows($fields_query); $i++) {
				...
				if ($adb->num_rows($ui10_modules_query) > 0) {
					...
					if ($queryPlanner->requireTable($crmentityRelModuleFieldTable, $matrix)) {
						$relquery.= " LEFT JOIN vtiger_crmentity AS $crmentityRelModuleFieldTable 
                                      ON $crmentityRelModuleFieldTable.crmid = $tab_name.$field_name AND 
                                         vtiger_crmentityRel$module$field_id.deleted=0";
					}
					for ($j = 0; $j < $adb->num_rows($ui10_modules_query); $j++) {
						$rel_mod = $adb->query_result($ui10_modules_query, $j, 'relmodule');
						$rel_obj = CRMEntity::getInstance($rel_mod);
		...
		$query .= " " . $relquery;
        $log->debug("Exting CRMEntity::generateReportsQuery(".$module.",".$queryPlanner.") method ...");
		return $query;
	}
	...
	function generateReportsSecQuery($module, $secmodule,$queryPlanner) {
        ...
        $log->debug("Entering CRMEntity::generateReportsSecQuery(".$module.",".$secmodule.",".$queryPlanner.") method ...");
		...
		if (!$queryPlanner->requireTable($tablename, $matrix)) {
            $log->debug("Exting CRMEntity::generateReportsSecQuery(".$module.",".$secmodule.",".$queryPlanner.") method ...");
			return '';
		}
	...
	function getListViewSecurityParameter($module) {
        ...
        $log->debug("Entering CRMEntity::getListViewSecurityParameter(".$module.") method ...");
		$tabid = getTabid($module);
		global $current_user;
		if ($current_user) {
			require('user_privileges/user_privileges_' . $current_user->id . '.php');
        ...
        $log->debug("Exting CRMEntity::getListViewSecurityParameter(".$module.") method ...");
		return $sec_query;
	}
	...
	function getRelationQuery($module, $secmodule, $table_name, $column_name, $queryPlanner) {
        global $log;
        die("getRelationQuery:".$module.":".$secmodule.":".$table_name.":".$column_name.":".$queryPlanner);
        $log->debug("Entering CRMEntity::getRelationQuery(".$module.",".$secmodule.",".$table_name.",".$column_name.",".$queryPlanner.") method ...");
		...
		$secQuery = "SELECT $table_name.* FROM $table_name 
                     INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid=$table_name.$column_name AND 
        ...
        $log->debug("Exting CRMEntity::getRelationQuery(".$module.",".$secmodule.",".$table_name.",".$column_name.",".$queryPlanner.") method ...");
		return $query;
	}
	...
	function add_related_to($module, $fieldname) {
        ...
        $log->debug("Entering CRMEntity::add_related_to(".$module.",".$fieldname.") method ...");
		$related_to = $this->column_fields[$fieldname];
		if (empty($related_to)) {
            $log->debug("Exting CRMEntity::add_related_to(".$module.",".$fieldname.") method ...");
			return false;
		}
	...
	function filterInactiveFields($module) {
        global $log;
        $log->debug("Entering ./data/CRMEntity.php::filterInactiveFields");
        global $log;

        $log->debug("Entering CRMEntity::filterInactiveFields(".$module.") method ...");
		if ($this->__inactive_fields_filtered) {
            $log->debug("Exting CRMEntity::filterInactiveFields(".$module.") method ...");
			return;
		}
	...
	function buildSearchQueryForFieldTypes($uitypes, $value=false) {
        ...
        $log->debug("Entering CRMEntity::buildSearchQueryForFieldTypes(".$uitypes.",".$value.") method ...");
		if (!is_array($uitypes))
        ...
        $log->debug("Exting CRMEntity::buildSearchQueryForFieldTypes(".$uitypes.",".$value.") method ...");
		return $query;
	}
	...
	function getOrderBy() {
		...
		$log->debug("Entering CRMEntity::getOrderBy() method ...");
